...
float Gyro_xd = 0;
float ACL_BufferX[100];
float ACL_BufferY[100];
float ACL_BufferZ[100];
bool data_ready;
...
//        Gyro_xd = GYRO_XYZ[0];
        ACL_BufferX[i] = ACL_XYZ[0];
        ACL_BufferY[i] = ACL_XYZ[1];
        ACL_BufferZ[i] = ACL_XYZ[2];
        i++;
        sprintf(outbuf, "ACL X:%.2f     ", ACL_XYZ[1]);
        LCD_WriteStringAtPos(outbuf, 1, 0);
        if(i == 50)
        {
            data_ready = true;
